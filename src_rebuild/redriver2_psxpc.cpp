...
void AutoPilotKeyHandler(int nKey, char down)
{
	if (!down)
		return;

	if (nKey == SDL_SCANCODE_G)
	{
		AlternarAutoPilot();
		printf("AutoPilot: %s\n", gAutoPilot ? "ON" : "OFF");
	}
	else if (nKey == SDL_SCANCODE_J)
	{
		AlternarModoCop();
		printf("ModoCop: %s\n", gModoCop ? "ON" : "OFF");
	}
...
void GameReleaseKeys(int nKey, char down)
{
	FreeCameraKeyboardHandler(nKey, down);
	AutoPilotKeyHandler(nKey, down);
}
...
int main(int argc, char** argv)
{
	...
	g_dbg_gameDebugKeys = GameReleaseKeys;
#else
