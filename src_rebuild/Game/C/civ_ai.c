...
int GetNextRoadInfo(CAR_DATA* cp, int randomExit, int* turnAngle, int* startDist, CIV_ROUTE_ENTRY* oldNode)
{
	...
	if (IS_JUNCTION_SURFACE(tmpNewRoad[0]))
	{
		...
		{
			int pularRestricao = (cp->id == player[0].playerCarId) &&
				((gAutoPilot && gAutoPilotDirReq != 0) || gModoCop == 2);
		...
		if (gAutoPilot && gAutoPilotDirReq != 0 && cp->id == player[0].playerCarId)
		{
			int prefIdx;
			if (gAutoPilotDirReq < 0)
				prefIdx = 0;
			else if (gAutoPilotDirReq == 2)
				prefIdx = 1;
			else
				prefIdx = 2;

			if (validExitIdx[prefIdx] != 42)
				bestExit = validExitIdx[prefIdx];
			else
			{
				rnd = Random2(0) % 3;
				do { bestExit = validExitIdx[rnd % 3]; rnd++; } while (bestExit == 42);
			}
			gAutoPilotDirReq = 0;
		}
		else if (gModoCop == 2 && cp->id == player[0].playerCarId && gFugitivo)
		{
			int fdx = gFugitivo->hd.where.t[0] - cp->hd.where.t[0];
			int fdz = gFugitivo->hd.where.t[2] - cp->hd.where.t[2];
			int cross = FIXEDH(cp->hd.where.m[0][0] * fdx + cp->hd.where.m[2][0] * fdz);
			int dot = FIXEDH(cp->hd.where.m[0][2] * fdx + cp->hd.where.m[2][2] * fdz);
			int prefIdx;

			if (dot > 0 && cross > -dot && cross < dot)
				prefIdx = 1;
			else if (cross < 0)
				prefIdx = 0;
			else
		...
		if (cp->ai.c.ctrlState != 8)
		{
			...
			if ((junctionFlags & 1) && cp != gFugitivo && !(gModoCop == 2 && cp->id == player[0].playerCarId))
			{
	...
	if (startDist != NULL)
	{
		if (GetSurfaceRoadInfo(&roadInfo, newRoad))
		{
			...
			if (cp != gFugitivo && !(gModoCop == 2 && cp->id == player[0].playerCarId))
...
int CheckChangeLanes(DRIVER2_STRAIGHT* straight, DRIVER2_CURVE* curve, int distAlongSegment, CAR_DATA* cp, int tryToPark)
{
	...
	if (cp->ai.c.ctrlState != 8 && (cp->ai.c.changeLaneIndicateCount == 0 || ((gAutoPilot || gModoCop) && (gAutoPilotDirReq != 0 || gTrocaFaixa != 0) && cp->id == player[0].playerCarId && cp->ai.c.changeLaneCount == 0)))
	{
		...
		pilotTgtLane = -1;
		if (gTrocaFaixa != 0 && cp->id == player[0].playerCarId && !tryToPark)
		{
			pilotTgtLane = (gTrocaFaixa < 0) ? currentLane - 1 : currentLane + 1;
			if (pilotTgtLane < 0 || pilotTgtLane >= roadWidthInLanes)
			{
				gTrocaFaixa = 0;
				return currentLane;
			}
			newLane = pilotTgtLane;
			gTrocaFaixa = 0;
		}
		else if (gAutoPilotDirReq != 0 && cp->id == player[0].playerCarId && !tryToPark)
		{
			int laneDir = straight ? ROAD_LANE_DIR(straight, currentLane) : ROAD_LANE_DIR(curve, currentLane);
			int lL = currentLane, rL = currentLane, i;
			for (i = 0; i < roadWidthInLanes; i++)
			{
				int ok = straight ? (ROAD_IS_AI_LANE(straight, i) && !ROAD_IS_PARKING_ALLOWED_AT(straight, i))
				                  : (ROAD_IS_AI_LANE(curve, i) && !ROAD_IS_PARKING_ALLOWED_AT(curve, i));
				int di = straight ? ROAD_LANE_DIR(straight, i) : ROAD_LANE_DIR(curve, i);
				if (ok && di == laneDir)
				{
					if (i < lL) lL = i;
					if (i > rL) rL = i;
			...
			pilotTgtLane = (gAutoPilotDirReq < 0) ? lL : rL;
			if (currentLane == pilotTgtLane)
				return currentLane;
			newLane = (currentLane < pilotTgtLane) ? currentLane + 1 : currentLane - 1;
		}
		...
		if (!canProceed && pilotTgtLane >= 0 && cp->id == player[0].playerCarId && (gAutoPilot || gModoCop))
			canProceed = 1;
			...
			if (retNode->pathType == 127)
			{
					...
					(travelDir < 0 && newNode->distAlongSegment < 0))
				{
						...
						(travelDir < 0 && start->distAlongSegment < 1))
					{
						...
						if (cp->ai.c.currentRoad == -1 || (cp->ai.c.thrustState == 3 && cp->ai.c.ctrlState == 7))
							break;
					}
					else
					{
	...
	while (carCnt < &car_data[MAX_CARS])
	{
		if (carCnt->controlType != CONTROL_TYPE_NONE)
		{
			...
			if (dy < 800)
			{
				...
				if (ABS(dx) < 0x2000 && ABS(dz) < 0x2000 && sqDist < 2500000)
				{
					count++;
...
void SetUpCivCollFlags(void)
{
	...
	for (cp0 = &car_data[MAX_CARS - 1]; cp0 >= car_data; cp0--)
	{
		...
		for (cp1 = &car_data[MAX_CARS];  cp1 >= car_data; cp1--)
		{
			...
			if (cp0->ai.c.thrustState != 3 &&
				!isTanner && cp1->controlType != CONTROL_TYPE_PLAYER &&
				(cp1->controlType == CONTROL_TYPE_CUTSCENE && gCurrentMissionNumber != 26 && ProxyBar.active == 0))
			{
				int dont;
				int rnd;
				rnd = Random2(0);
...
int CivControl(CAR_DATA* cp)
{
	...
	if (cp->controlType == CONTROL_TYPE_CIV_AI)
	{
		...
		if (cp->ai.c.thrustState == 3 && cp->ai.c.ctrlState == 7 &&
			cp->id == player[0].playerCarId && (gAutoPilot || gModoCop))
		{
			DRIVER2_ROAD_INFO rdi;
			int surfIdx = GetSurfaceIndex((VECTOR*)cp->hd.where.t);

			if (!IS_ROAD_SURFACE(surfIdx))
			{
				VECTOR tmpPos;
				tmpPos.vx = cp->hd.where.t[0];
				tmpPos.vy = cp->hd.where.t[1];
				tmpPos.vz = cp->hd.where.t[2];
				surfIdx = BuscarEstradaProxima(&tmpPos);
			}

			if (IS_ROAD_SURFACE(surfIdx) && GetSurfaceRoadInfo(&rdi, surfIdx))
			{
				cp->ai.c.currentRoad = surfIdx;
				cp->ai.c.currentLane = GetLaneByPositionOnRoad(&rdi, (VECTOR*)cp->hd.where.t);
				cp->ai.c.thrustState = 0;
				cp->ai.c.ctrlState = 0;
				cp->ai.c.ctrlNode = NULL;
				cp->ai.c.turnNode = -1;
				cp->ai.c.changeLaneCount = 0;
				cp->ai.c.changeLaneIndicateCount = 0;
				InitNodeList(cp, NULL);
				cp->ai.c.pnode = &cp->ai.c.targetRoute[0];
				cp->ai.c.currentNode = 0;
			}
		}
