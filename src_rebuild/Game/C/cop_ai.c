...
void CopControl1(CAR_DATA *cp)
{
	...
	if (cp->ai.p.recoveryTimer < 1)
	{
		...
		if (cp->ai.p.close_pursuit == 0)
		{
			if (cp->ai.p.alvo)
			{
				targetPoint.vx = targetVehicle->hd.where.t[0];
				targetPoint.vz = targetVehicle->hd.where.t[2];
			}
			else
			{
	...
	if (targetFound) 
	{
		...
		{			
			...
			if (cp->ai.p.alvo)
				cp->ai.p.desiredSpeed = FIXEDH(speed1[gCopDifficultyLevel] * gCopDesiredSpeedScale);
			else
	...
	if (cp->ai.p.justPinged == 1)
	{
		if (cp->ai.p.alvo)
		{
			int adx = cp->ai.p.alvo->hd.where.t[0] - cp->hd.where.t[0];
			int adz = cp->ai.p.alvo->hd.where.t[2] - cp->hd.where.t[2];
			cp->hd.direction = ratan2(adx, adz) & 0xfff;
		}
		else
		{
...
{
	CAR_DATA* alvoBkp = NULL;
	if (cp->ai.p.alvo)
	{
		if (cp->ai.p.alvo->controlType == CONTROL_TYPE_NONE)
		{
			cp->ai.p.alvo = NULL;
			cp->ai.p.dying = 1;
		}
		...
		{
			alvoBkp = targetVehicle;
			targetVehicle = cp->ai.p.alvo;
		}
	}
...
static int gPersegTimer = 0;
CAR_DATA* gFugitivo = NULL;
CAR_DATA* gInfrator = NULL;
static int gPersegDuracao = 0;
static int gPersegRespawnTimer = 0;

#define PERSEG_INTERVALO 2700
#define PERSEG_DURACAO_MAX (30 * 60)
#define PERSEG_DIST_MAX 33000

static int gPersegSpeedBkp = 0;
static int gPersegPowerBkp = 0;
static int gPersegBatterBkp = 0;
static int gPersegImmortalBkp = 0;
static int gPersegDiffBkp = 0;

void EncerrarPerseguicaoCivil()
{
	CAR_DATA* cp;
	int i, d;

	for (i = 0, cp = car_data; i < MAX_CARS; i++, cp++)
	{
		if (cp->controlType == CONTROL_TYPE_PURSUER_AI && cp->ai.p.alvo == gFugitivo)
		{
			if (gModoCop == 2 && cp->id == player[0].playerCarId)
				continue;
	...
	if (gFugitivo)
	{
		gFugitivo->controlType = CONTROL_TYPE_NONE;
		numCivCars--;
	}

	gCopDesiredSpeedScale = gPersegSpeedBkp;
	gCopMaxPowerScale = gPersegPowerBkp;
	gBatterPlayer = gPersegBatterBkp;
	gCopData.immortal = gPersegImmortalBkp;
	gCopDifficultyLevel = gPersegDiffBkp;

	gFugitivo = NULL;
	gPersegDuracao = 0;
}

void AtualizarPerseguicaoCivil()
{
	if (!gModoCop)
		return;
	...
	if (gFugitivo)
	{
		if (gFugitivo->controlType != CONTROL_TYPE_CIV_AI)
		{
		...
		gPersegDuracao++;
		if (gPersegDuracao >= PERSEG_DURACAO_MAX)
		{
			EncerrarPerseguicaoCivil();
			return;
		}

		if (++gPersegRespawnTimer >= 30)
		{
			CAR_DATA* cp;
			int i, qtdCops = 0;

			gPersegRespawnTimer = 0;
			...
			for (i = 0, cp = car_data; i < MAX_CARS; i++, cp++)
			{
				...
				dx = cp->hd.where.t[0] - gFugitivo->hd.where.t[0];
				dz = cp->hd.where.t[2] - gFugitivo->hd.where.t[2];
				adx = dx < 0 ? -dx : dx;
				adz = dz < 0 ? -dz : dz;

				if (adx + adz > PERSEG_DIST_MAX)
				{
					cp->controlType = CONTROL_TYPE_NONE;
					numCopCars--;
				}
			...
			for (i = 0, cp = car_data; i < MAX_CARS && qtdCops < 60; i++, cp++)
			{
				int dx, dz, adx, adz;
				...
				cp->ap.model = 3;
				cp->ap.palette = 0;
				{ int d; for (d = 0; d < 6; d++) cp->ap.damage[d] = 0; }
				cp->totalDamage = 0;
	...
	{
		CAR_DATA* cp;
		CAR_DATA* melhor = NULL;
		int i, qtdCiv = 0;

		if (gInfrator)
		{
			if (gInfrator->controlType != CONTROL_TYPE_CIV_AI)
			{
				gInfrator = NULL;
				return;
			}
		...
		{
			gPersegTimer++;
			if (gPersegTimer < PERSEG_INTERVALO)
				return;
		...
		gFugitivo = melhor;
		gFugitivo->ai.c.maxSpeed = 255;
		gPersegDuracao = 0;

		gPersegSpeedBkp = gCopDesiredSpeedScale;
		gPersegPowerBkp = gCopMaxPowerScale;
		gPersegBatterBkp = gBatterPlayer;
		gPersegImmortalBkp = gCopData.immortal;
		gPersegDiffBkp = gCopDifficultyLevel;

		gCopDesiredSpeedScale = 9144;
		gCopMaxPowerScale = 9144;
		gBatterPlayer = 1;
		gCopData.immortal = 1;
		gCopDifficultyLevel = 4;

		int qtdCops = 0;

		for (i = 0, cp = car_data; i < MAX_CARS && qtdCops < 40; i++, cp++)
		{
			int dx, dz, adx, adz;
		...
		if (qtdCops < 5)
		{
			int j;
			CAR_DATA* rcp;
			for (j = 0, rcp = car_data; j < MAX_CARS; j++, rcp++)
			{
				if (rcp->controlType == CONTROL_TYPE_PURSUER_AI && rcp->ai.p.alvo == gFugitivo)
				{
					if (gModoCop == 2 && rcp->id == player[0].playerCarId)
						continue;
					int rd;
					for (rd = 0; rd < 6; rd++)
						rcp->ap.damage[rd] = 0;
					rcp->totalDamage = 0;
					rcp->ap.needsDenting = 1;
					if (InitCivState(rcp, NULL))
					{
						numCopCars--;
						numCivCars++;
					}
					...
					{
						rcp->controlType = CONTROL_TYPE_NONE;
						numCopCars--;
					}
				}
			}

			gFugitivo->ai.c.maxSpeed = speedLimits[1];
			gFugitivo = NULL;
