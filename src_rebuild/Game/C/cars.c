...
void DrawCar(CAR_DATA* cp, int view)
{
	...
	if (cp->controlType == CONTROL_TYPE_PLAYER)
	{
		...
		{
			if (gAutoPilotState.thrustState != 3)
			{
				if (gAutoPilotState.turnNode != -1)
					AddIndicatorLight(cp, gAutoPilotState.turnDir);

				if (gAutoPilotState.changeLaneIndicateCount != 0 && gAutoPilotState.turnNode == -1)
					AddIndicatorLight(cp, gAutoPilotState.changeLane);
			}

			if (gAutoPilotState.brakeLight != 0)
				AddBrakeLight(cp);
		}
		else if (gModoCop && cp->id == player[0].playerCarId)
		{
			if (gModoCopState.thrustState != 3)
			{
				if (gModoCopState.turnNode != -1)
					AddIndicatorLight(cp, gModoCopState.turnDir);

				if (gModoCopState.changeLaneIndicateCount != 0 && gModoCopState.turnNode == -1)
					AddIndicatorLight(cp, gModoCopState.changeLane);
			}

			if (gModoCopState.brakeLight != 0)
				AddBrakeLight(cp);
		}
		else
		{
	...
	if (gLightsOn && !lightsOnDelay[cp->id])
	{
		...
		else if (gAutoPilot && cp->id == player[0].playerCarId)
		{
			if (gAutoPilotState.thrustState != 3 || (gAutoPilotState.ctrlState != 5 && gAutoPilotState.ctrlState != 7 && gAutoPilotState.ctrlState != 8))
				AddNightLights(cp);
		}
		else if (gModoCop && cp->id == player[0].playerCarId)
		{
			if (gModoCopState.thrustState != 3 || (gModoCopState.ctrlState != 5 && gModoCopState.ctrlState != 7 && gModoCopState.ctrlState != 8))
				AddNightLights(cp);
		}
