...
void GlobalTimeStep(void)
{
	...
	for (i = 0; i < num_active_cars; i++)
	{
		...
		if (!CARBM_IS_ZERO(cp->hd.mayBeColliding))
		{
			continue;
		}
	...
	for(subframe = 0; subframe < 4; subframe++) 
	{
		for (RKstep = 0; RKstep < 2; RKstep++)
		{
			for (i = 0; i < num_active_cars; i++)
			{
				...
				CARBM_COPY(mayBeCollidingBits, cp->hd.mayBeColliding);

				// if has any collision, process with double precision
				if (CARBM_IS_ZERO(mayBeCollidingBits))
				{
					continue;
				}
				...
				for (j = 0; j < i; j++)
				{
					...
					if (!CARBM_TST(mayBeCollidingBits, CAR_INDEX(c1)) || (c1->hd.speed == 0 && cp->hd.speed == 0))
					{
						continue;
					}
					...
					if (howHard > 0)
					{
						...
						if (gModoCop && !gInfrator && !gFugitivo && howHard > 0x1000)
						{
							int cpIsCop = (cp->controlFlags & CONTROL_FLAG_COP) ||
								(gModoCop && cp->id == player[0].playerCarId);
							int c1IsCop = (c1->controlFlags & CONTROL_FLAG_COP) ||
								(gModoCop && c1->id == player[0].playerCarId);

							if (cpIsCop && c1->controlType == CONTROL_TYPE_CIV_AI &&
								!(c1->controlFlags & CONTROL_FLAG_COP))
								gInfrator = c1;
							else if (c1IsCop && cp->controlType == CONTROL_TYPE_CIV_AI &&
...
void CheckCarToCarCollisions(void)
{
	...
	do {
		...
		{
		{
			CARBM_ZERO(cp->hd.mayBeColliding);
			CARBM_SET(cp->hd.mayBeColliding, CAR_COLLIDE_PLAYER_BIT);
		}
		}
	...
	do
	{
		...
		while (loop2 < MAX_CARS)
		{
			...
			{
				CARBM_SET(car_data[loop1].hd.mayBeColliding, loop2);
				CARBM_SET(car_data[loop2].hd.mayBeColliding, loop1);
			}
		...
		if (gShowCollisionDebug == 2 && car_data[loop1].controlType != CONTROL_TYPE_NONE)
		{
			...
			CVECTOR bbcol = !CARBM_IS_ZERO(car_data[loop1].hd.mayBeColliding) ? rrcv : yycv;
...
void CheckCarEffects(CAR_DATA* cp, int player_id)
{
	...
	if (skidsound != 0 && ((cp->hd.wheel[1].surface & 0x80) == 0 || (cp->hd.wheel[3].surface & 0x80) == 0))
	{
		if (gWeather - 1U < 2)
			desired_skid = -1;
		else if (cp->id != player[0].playerCarId)
			desired_skid = 7;
	}
