...
void SetupCivJump(LPPEDESTRIAN pPed, CAR_DATA* cp)
{
	...
	if (pPed->type != PED_ACTION_JUMP)
	{
		...
		if (cp == &car_data[player[0].playerCarId] && !gAutoPilot)
		{
		...
		else if (gModoCop && cp && cp->controlType == CONTROL_TYPE_CIV_AI &&
			!(cp->controlFlags & CONTROL_FLAG_COP) && !gInfrator && !gFugitivo)
		{
			gInfrator = cp;
		}
	}
...
void set_coll_box(int index, CAR_DATA* cp, int offset)
{
	...
	isPlayerCar = (cp == &car_data[player[0].playerCarId]) && !gAutoPilot;
