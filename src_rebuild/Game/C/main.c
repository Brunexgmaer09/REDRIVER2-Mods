...
int gAutoPilot = 0;
int gAutoPilotDirReq = 0;
int gTrocaFaixa = 0;
static int gUltTapDir = 0;
static int gUltTapFrame = 0;
static int gUltDirDown = 0;
CIV_STATE gAutoPilotState;
static char* gAutoPilotPadId = NULL;

int gModoCop = 0;
CIV_STATE gModoCopState;
COP gModoCopPState;
static char* gModoCopPadBkp = NULL;

int gTornado = 0;
static VECTOR gTornadoPos;
static int gTornadoAng = 0;
static int gTornadoDir = 0;
static int gTornadoToggleCD = 0;
...
#define CLIMA_TOTAL 5
#define CLIMA_INTERVALO (30 * 60 * 3)

static int gClimaTimer = 0;
static int gClimaIdx = 0;

static const int tblTempo[CLIMA_TOTAL] = { TIME_DAY, TIME_DUSK, TIME_NIGHT, TIME_DAWN, TIME_DAY };
static const int tblClima[CLIMA_TOTAL] = { WEATHER_NONE, WEATHER_NONE, WEATHER_NONE, WEATHER_NONE, WEATHER_RAIN };
...
void AtualizarCicloClima(void)
{
	int i;

	gClimaTimer++;

	if (gClimaTimer < CLIMA_INTERVALO)
		return;

	gClimaTimer = 0;
	gClimaIdx = (gClimaIdx + 1) % CLIMA_TOTAL;

	gTimeOfDay = tblTempo[gClimaIdx];
	gWeather = tblClima[gClimaIdx];
	DawnCount = 0;
	gNight = (gTimeOfDay >= TIME_NIGHT) ? 1 : 0;
...
void StepSim(void)
{
	...
	if (requestStationaryCivCar != 1 && requestRoadblock == 0)
	{
		...
		if ((gAutoPilot || gModoCop == 1) && player[0].playerCarId >= 0)
		{
			CAR_DATA* apCar = &car_data[player[0].playerCarId];
			char* tmpPad = apCar->ai.padid;

			if (gAutoPilot)
				apCar->ai.c = gAutoPilotState;
			else
				apCar->ai.c = gModoCopState;

			apCar->controlType = CONTROL_TYPE_CIV_AI;

			SetUpCivCollFlags();

			if (gAutoPilot)
				gAutoPilotState = apCar->ai.c;
			else
				gModoCopState = apCar->ai.c;

			apCar->controlType = CONTROL_TYPE_PLAYER;
			apCar->ai.padid = tmpPad;
		}
		else
		{
			SetUpCivCollFlags();
	...
	while (cp < &car_data[MAX_CARS])
	{
		switch (cp->controlType)
		{
			case CONTROL_TYPE_PLAYER:
				...
				if (gAutoPilot && cp->id == player[0].playerCarId)
				{
					if ((t0 & CAR_PAD_LEAVECAR) == CAR_PAD_LEAVECAR)
					{
						DesativarAutoPilot();
					...
					{
						char* tmpPad = cp->ai.padid;
						int dirAtual = 0;

						if (t0 & CAR_PAD_LEFT)
							dirAtual = -1;
						else if (t0 & CAR_PAD_RIGHT)
							dirAtual = 1;

						if (dirAtual != 0 && gUltDirDown == 0)
						{
							if (dirAtual == gUltTapDir && (CameraCnt - gUltTapFrame) < 12)
								gTrocaFaixa = dirAtual;
							gUltTapDir = dirAtual;
							gUltTapFrame = CameraCnt;
						}
						gUltDirDown = dirAtual;

						if (dirAtual != 0)
							gAutoPilotDirReq = dirAtual;
						else if (t0 & CAR_PAD_UP)
							gAutoPilotDirReq = 2;
						...
						cp->handbrake = 0;
						cp->wheelspin = 0;
						cp->ai.c = gAutoPilotState;

						CivControl(cp);

						gAutoPilotState = cp->ai.c;
						cp->controlType = CONTROL_TYPE_PLAYER;
						cp->ai.padid = tmpPad;
					}
				}
				else if (gModoCop == 1 && cp->id == player[0].playerCarId)
				{
					if ((t0 & CAR_PAD_LEAVECAR) == CAR_PAD_LEAVECAR)
					{
						DesativarModoCop();
					...
					else
					{
						...
						cp->handbrake = 0;
						cp->wheelspin = 0;
						cp->ai.c = gModoCopState;

						CivControl(cp);

						gModoCopState = cp->ai.c;
						cp->controlType = CONTROL_TYPE_PLAYER;
						cp->ai.padid = tmpPad;
					}
				}
				else if (gModoCop == 2 && cp->id == player[0].playerCarId)
				{
					if ((t0 & CAR_PAD_LEAVECAR) == CAR_PAD_LEAVECAR)
					{
						DesativarModoCop();
					...
					else if (gFugitivo)
					{
						...
						cp->hndType = 4;
						cp->ai.p = gModoCopPState;
						cp->ai.p.alvo = gFugitivo;

						CopControl(cp);

						gModoCopPState = cp->ai.p;
						cp->controlType = CONTROL_TYPE_PLAYER;
						cp->ai.padid = gModoCopPadBkp;
					}
				}
				else
				{
		...
		if (gModoCop == 2 && gFugitivo && cp->controlType == CONTROL_TYPE_PLAYER)
			BoostCopPlayer(cp);
	...
	if (gAutoPilot && player[0].playerCarId >= 0)
		car_data[player[0].playerCarId].hndType = 0;
...
void DesativarAutoPilot()
{
	if (!gAutoPilot)
		return;

	if (player[0].playerCarId >= 0 && gAutoPilotPadId)
	{
		CAR_DATA* cp = &car_data[player[0].playerCarId];
		cp->controlType = CONTROL_TYPE_PLAYER;
		cp->ai.padid = gAutoPilotPadId;
	}

	gAutoPilot = 0;
	gAutoPilotDirReq = 0;
	gAutoPilotPadId = NULL;
}

int BuscarEstradaProxima(VECTOR* pos)
{
	int dx, dz, raio;
	int surfId, melhorSurf, melhorDist, dist;
	VECTOR tmp;
	VECTOR melhorPos;

	for (raio = 16; raio <= 128; raio += 16)
	{
		melhorDist = 999999;
		melhorSurf = -1;

		for (dz = -raio; dz <= raio; dz++)
		{
			for (dx = -raio; dx <= raio; dx++)
			{
				if (abs(dx) < raio - 15 && abs(dz) < raio - 15)
					continue;

				if (dx == 0 && dz == 0)
					continue;

				tmp.vx = pos->vx + dx * 512;
				tmp.vy = pos->vy;
				tmp.vz = pos->vz + dz * 512;

				surfId = GetSurfaceIndex(&tmp);
				if (IS_ROAD_SURFACE(surfId))
				{
					dist = abs(dx) + abs(dz);
					if (dist < melhorDist)
					{
...
void AlternarAutoPilot()
{
	if (player[0].playerType != 1 || player[0].playerCarId < 0)
		return;
	...
	gAutoPilotPadId = cp->ai.padid;

	int posX = cp->hd.where.t[0];
	int posZ = cp->hd.where.t[2];

	if (!IS_ROAD_SURFACE(GetSurfaceIndex((VECTOR*)cp->hd.where.t)))
		BuscarEstradaProxima((VECTOR*)cp->hd.where.t);

	if (!InitCivState(cp, NULL))
	{
		cp->controlType = CONTROL_TYPE_PLAYER;
		cp->ai.padid = gAutoPilotPadId;
		gAutoPilotPadId = NULL;
		cp->hd.where.t[0] = posX;
		cp->hd.where.t[2] = posZ;
		return;
	}
...
void DesativarModoCop()
{
	if (!gModoCop)
		return;

	if (player[0].playerCarId >= 0 && gModoCopPadBkp)
	{
		CAR_DATA* cp = &car_data[player[0].playerCarId];
		cp->controlType = CONTROL_TYPE_PLAYER;
		cp->ai.padid = gModoCopPadBkp;
	...
	if (gFugitivo)
		EncerrarPerseguicaoCivil();

	gModoCop = 0;
	gModoCopPadBkp = NULL;
}

void AlternarModoCop()
{
	if (player[0].playerType != 1 || player[0].playerCarId < 0)
		return;
...
void DesvirarCarro(CAR_DATA* cp)
{
	int dir = cp->hd.direction;
	int sn = RSIN(dir / 2);
	int cs = RCOS(dir / 2);

	cp->st.n.orientation[0] = 0;
	cp->st.n.orientation[1] = sn;
	cp->st.n.orientation[2] = 0;
	...
	cp->st.n.fposition[1] -= 600;

	RebuildCarMatrix(&cp->st, cp);
}

void BoostCopPlayer(CAR_DATA* cp)
{
	if (cp->thrust != 0)
		cp->thrust = cp->thrust * 5500 / 4096;
}

void AtualizarModoCop()
{
	CAR_DATA* cp;
	int d;

	if (!gModoCop || player[0].playerCarId < 0)
		return;

	cp = &car_data[player[0].playerCarId];

	if (cp->hd.where.m[1][1] < 0)
		DesvirarCarro(cp);

	if (gModoCop == 1 && gFugitivo)
	{
	...
	else if (gModoCop == 2 && !gFugitivo)
	{
		...
		posX = cp->hd.where.t[0];
		posZ = cp->hd.where.t[2];
		...
		if (InitCivState(cp, NULL))
		{
			...
			cp->hd.where.t[0] = posX;
			cp->hd.where.t[2] = posZ;
			gModoCopState.maxSpeed = speedLimits[1];
			cp->hndType = 0;
			gModoCop = 1;
		}
...
#define TORNADO_RAIO 6000
#define TORNADO_RAIO_SQ ((long long)TORNADO_RAIO * TORNADO_RAIO)
#define TORNADO_ALT 3000
#define TORNADO_NIVEIS 5
#define TORNADO_BASE_RAIO 600
#define TORNADO_TOPO_RAIO 100
#define TORNADO_VEL 20

static void SpawnarFumacaTornado()
{
	int nv, p;
	VECTOR pos;
	VECTOR drift;

	for (nv = 0; nv < TORNADO_NIVEIS; nv++)
	{
		int alt = (nv * TORNADO_ALT) / TORNADO_NIVEIS;
		int raio = TORNADO_BASE_RAIO - (nv * (TORNADO_BASE_RAIO - TORNADO_TOPO_RAIO)) / TORNADO_NIVEIS;
		int angBase = gTornadoAng + nv * 820;

		for (p = 0; p < 3; p++)
		{
			int ang = angBase + p * 1365;
			pos.vx = gTornadoPos.vx + FIXEDH(RSIN(ang & 0xfff) * raio);
			pos.vy = gTornadoPos.vy - alt;
			pos.vz = gTornadoPos.vz + FIXEDH(RCOS(ang & 0xfff) * raio);

			drift.vx = FIXEDH(-RCOS(ang & 0xfff) * 40);
			drift.vy = -30;
			drift.vz = FIXEDH(RSIN(ang & 0xfff) * 40);

			Setup_Smoke(&pos, 40 + nv * 15, 200 + nv * 40, SMOKE_BLACK, 0, &drift, 1);
		}
	}
}

static void AplicarForcaTornado()
{
	int i;
	CAR_DATA* cp;
	int plId = player[0].playerCarId;
	...
	for (i = 0; i < num_active_cars; i++)
	{
		...
		int dx = cp->hd.where.t[0] - gTornadoPos.vx;
		int dz = cp->hd.where.t[2] - gTornadoPos.vz;
		long long distSq = (long long)dx * dx + (long long)dz * dz;

		if (distSq >= TORNADO_RAIO_SQ || distSq == 0)
			continue;

		int dist = 1;
		{
			long long tmp = distSq;
			int d = 0;
			while (tmp > 0) { tmp >>= 2; d++; }
			dist = 1 << d;
			if (dist > 1)
		...
		int txf = FIXEDH(-ndz * str);
		int tzf = FIXEDH(ndx * str);
		int rxf = FIXEDH(-ndx * (str >> 1));
		int rzf = FIXEDH(-ndz * (str >> 1));

		cp->st.n.linearVelocity[0] += txf + rxf;
		cp->st.n.linearVelocity[1] += str * 2;
		cp->st.n.linearVelocity[2] += tzf + rzf;

		cp->hd.aacc[0] += (tzf + rzf) >> 1;
		cp->hd.aacc[1] += str >> 2;
		cp->hd.aacc[2] += -(txf + rxf) >> 1;
	}
}

void AtualizarTornado()
{
	if (gTornadoToggleCD > 0)
		gTornadoToggleCD--;

	if (!gTornado)
		return;

	gTornadoAng = (gTornadoAng + 80) & 0xfff;

	gTornadoPos.vx += FIXEDH(RSIN(gTornadoDir & 0xfff) * TORNADO_VEL);
	gTornadoPos.vz += FIXEDH(RCOS(gTornadoDir & 0xfff) * TORNADO_VEL);

	SpawnarFumacaTornado();
	AplicarForcaTornado();
}

void AlternarTornado()
{
	if (player[0].playerType != 1 || player[0].playerCarId < 0)
		return;
	...
	gTornadoPos.vx = cp->hd.where.t[0] + FIXEDH(RSIN(cp->hd.direction) * 4000);
	gTornadoPos.vy = cp->hd.where.t[1];
	gTornadoPos.vz = cp->hd.where.t[2] + FIXEDH(RCOS(cp->hd.direction) * 4000);
	gTornadoAng = 0;
	gTornadoDir = cp->hd.direction;
	gTornado = 1;
}
...
void RenderGame2(int view)
{
	...
	for (i = 0; i < 2; i++)
	{
		if (player[i].playerCarId >= 0 &&
			...
			(player[i].horn.on != 0 || (gModoCop == 2 && i == 0)))
		{
	...
	if (gAutoPilot)
	{
		SetTextColour(100, 220, 100);
		PrintStringRightAligned("Autopilot ON", 310, 8);
	}

	if (gModoCop == 1 || gModoCop == 2)
	{
		SetTextColour(100, 180, 255);
		PrintStringRightAligned("Cop Mode ON", 310, gAutoPilot ? 20 : 8);
	}
	...
	if (gModoCop == 2 && gFugitivo)
	{
		VECTOR tvCop;
		tvCop.vx = gFugitivo->hd.where.t[0];
		tvCop.vz = gFugitivo->hd.where.t[2];
		tvCop.vy = -gFugitivo->hd.where.t[1];
		gDraw3DArrowBlue = 0;
		Draw3DTarget(&tvCop, 0x1);
	}
