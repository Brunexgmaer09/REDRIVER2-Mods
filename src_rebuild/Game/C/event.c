...
{
	if (!CARBM_TST(carsOnBoat, CAR_INDEX(cp)))
		return;
...
void StepEvents(void)
{
	...
	CELL_OBJECT* cop;
	CAR_DATA* cp;
	CarBitmask onBoatLastFrame;
	int dist;
	int thisCamera, otherCamera;

	CARBM_COPY(onBoatLastFrame, carsOnBoat);
	ev = firstEvent;
		...
		if (ev->flags & 2)
		{
			if (ev->flags & 0x40)
			{
				...
				if (player[0].playerType == 2 && OnBoat((VECTOR*)player[0].pos, ev, &dist))
					{
					CARBM_SET(carsOnBoat, TANNER_COLLIDER_CARID);
					CARBM_SET(carsOnBoat, TANNER_COLLIDER_CARID + 1);
				}
			...
			// move cars on boats
			if ((ev->flags & 0x40) && (!CARBM_IS_ZERO(carsOnBoat) || !CARBM_IS_ZERO(onBoatLastFrame)))
			{
				...
				for (i = 0; i < MAX_CARS + 1; i++)
				{
					...
					if (CARBM_TST(carsOnBoat, i))
					{
						...
						if (i == TANNER_COLLIDER_CARID)
						{
							...
							CARBM_CLR(carsOnBoat, TANNER_COLLIDER_CARID);
						}
						else if (!CARBM_TST(onBoatLastFrame, i))
						{
							vel->vx -= speed.x * 4096;
							vel->vz -= speed.z * 4096;
						}
					}
					else if (vel && CARBM_TST(onBoatLastFrame, i))
					{
						vel->vx += speed.x * 4096;
						vel->vz += speed.z * 4096;
					}
