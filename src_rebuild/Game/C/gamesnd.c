...
ushort GetEngineRevs(CAR_DATA* cp)
{
	...
	acc = cp->thrust;
	type = (cp->controlType == CONTROL_TYPE_CIV_AI || ((gAutoPilot || gModoCop) && cp->controlType == CONTROL_TYPE_PLAYER));
...
void ControlCarRevs(CAR_DATA* cp)
{
	...
	cp->hd.revs = newRevs;
	if (player_id != -1 && !gAutoPilot && gModoCop != 1)
	{
...
char force_idle[MAX_CARS] = { 0 };
char force_siren[MAX_CARS] = { 0 };
...
void DoDopplerSFX(void)
{
	...
	for (i = 0; i < MAX_CARS; i++)
	{
		...
		{
			if ((gAutoPilot || gModoCop) && car_ptr->controlType == CONTROL_TYPE_PLAYER && i == player[0].playerCarId)
			{
				dist = jsqrt(dx * dx + dz * dz) + 0x6000;
			}
			else if (car_ptr->controlType == CONTROL_TYPE_CIV_AI && car_ptr->ai.c.ctrlState != 5 && car_ptr->ai.c.ctrlState != 7)
			{
				dist = jsqrt(dx * dx + dz * dz) + 0x6000;
			}
	...
	for (i = 0; i < num_noisy_cars && sirens < 2; i++)
	{
		...
		CARBM_SET(car_flags, indexlist[i]);
	...
	{
		int siren;
		siren = CARBM_TST(car_flags, siren_noise[i].car) != 0;

		siren_noise[i].in_use = siren;
		if (siren) CARBM_CLR(car_flags, siren_noise[i].car);
	...
	{
		if (CARBM_TST(car_flags, indexlist[i]))
		{
			car = indexlist[i];
	...
	for (j = 0; j < MAX_CAR_NOISES; j++)
	{
		...
		noise = CARBM_TST(car_flags, car_noise[j].car) != 0;
		car_noise[j].in_use = noise;

		if (noise) CARBM_CLR(car_flags, car_noise[j].car);
	}
	...
	for (j = 0; j < MAX_CAR_NOISES; j++)
	{
		...
		if (car_data[car].controlType == CONTROL_TYPE_CIV_AI || ((gAutoPilot || gModoCop) && car == player[0].playerCarId))
			volume = -7000;
		else
			volume = -6250;
...
void SoundTasks(void)
{
	...
	while (i < NumPlayers)
	{
		...
		// car engine sounds
		if (cp && (gAutoPilot || gModoCop == 1) && i == 0)
		{
			chan = i * 3;
