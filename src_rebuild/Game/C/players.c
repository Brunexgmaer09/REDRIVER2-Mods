...
void ChangePedPlayerToCar(int playerID, CAR_DATA *newCar)
{
	...
	if(lPlayer->padid >= 0)
	{
		...
		{
			if (newCar->controlFlags & (CONTROL_FLAG_COP | CONTROL_FLAG_COP_SLEEPING))
			{
				newCar->felonyRating = 0;
				pedestrianFelony = 0;
				newCar->controlFlags &= ~(CONTROL_FLAG_COP | CONTROL_FLAG_COP_SLEEPING);
			}
			else if (gCurrentMissionNumber != 32 && MissionHeader->residentModels[newCar->ap.model] == 0)
			{
